...
function shouldSquashOperations(
  operations
  /*: List<SlateOperation>*/
) {
...
function recalculateOperations(
  ...
  const recalculatedOperations = [] // Run through all the remote operations and check for remove_node, insert_node, move_node

  item.operations.forEach(itemOp => {
    let checkedOp = itemOp
    const pIndex = 0 // console.log('Checking', checkedOp.toJSON())
...
export default function UndoRedoPlugin(
  ...
  function handleUndoItem(
    ...
    if (operations.size > 0) {
      const nextItem = stack.undo.slice(-1)[0] // Check if we should squash this undo step into the next
  ...
  function handleRedoItem(
    ...
    if (operations.size > 0) {
      const nextItem = stack.redo.slice(-1)[0] // Check if we should squash this redo step into the next
