...
export default function ListItemOnEnterKeyPlugin(
  /*: Options*/
  options = {}
) {
  ...
  return {
    ...
    onKeyDown(
      ...
      const {value} = editor
      const {document, startBlock, selection} = value // Only do listItem nodes
      ...
      if (level > 1) {
        blockToInsert = {...defaultBlock, data: startBlock.data.toObject()}
