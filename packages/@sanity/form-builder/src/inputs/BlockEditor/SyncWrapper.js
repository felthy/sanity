...
function isDeprecatedBlockSchema(
  type
  /*: Type*/
) {
...
function isDeprecatedBlockValue(
  value
  /*: ?(FormBuilderValue[])*/
) {
...
export default withPatchSubscriber(
  class SyncWrapper extends React.Component
  /*:: <Props, State>*/
  {
    static defaultProps = {
      readOnly: false,
    ...
    _pendingLocalChanges =
      /*: [{operation: SlateOperation, patches: Patch[]}][]*/
      []
    /*:: operationToPatches: (
    ...
    handleChangeSet = (
      ...
      const {operations, isRemote, selection, callback} = changeSet // Add undo step for local changes
