...
function findTextNodeFromPathKey(
  blockNode,
  /*: SlateNode*/
  pathKey
  /*: Path*/
...
export default function createPatchesToChange(
  ...
  function unsetPatch(
    ...
    const isDirectlyTargeted =
      patch.path.findIndex(part => part._key && part._key === lastKey) === patch.path.length - 1 // If it is targeting a node in our document directly, just remove that node
  ...
  function replaceValue(
    ...
    if (snapshot) {
      const fragment = blocksToEditorValue(snapshot, blockContentType) // Store the old selection
  ...
  function patchSpanText(
    patch,
    /*: InsertPatch | SetPatch | DiffMatchPatch*/
    editor,
    /*: SlateEditor*/
    node,
    /*: SlateNode*/
    ...
    const workTextNode = textNode.toJSON({
      preserveKeys: true
    })
    ...
    if (patch.type === 'set') {
      ...
      if (textNode.leaves.size === 1) {
        ...
        if (valueIsString) {
          ...
          marks = Mark.createSet(
            patch.value.marks.map(mark => ({
              type: mark
            }))
          )
        }

        editor.replaceNodeByPath(
          textPath,
          Text.create({
            text: patchText,
            marks
  ...
  return function patchToOperations(
    patch,
    /*: Patch*/
    editorValue
    /*: SlateValue*/
    ...
    const isMarkDefPatches = !isVoidRootBlock && patch.path[1] === 'markDefs' // Patches working inside blocks needs to be treated a bit special,
    ...
    if (patch.path.length > 1) {
      if (isMarkDefPatches) {
        ...
        const node = findLastKnownEditorNodeInPath(rootBlock, patch.path) // eslint-disable-next-line max-depth
